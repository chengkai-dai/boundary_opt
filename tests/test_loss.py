...
def test_length_smoothness_gradient_matches_finite_difference() -> None:
    ...
    loss, gradient = length_smoothness_loss_and_gradient(
        field, mesh.faces, gradient_basis, weights
    )
    step = 1.0e-6
    forward = length_smoothness_loss_and_gradient(
        field + step * direction, mesh.faces, gradient_basis, weights
    )[0]
    backward = length_smoothness_loss_and_gradient(
        field - step * direction, mesh.faces, gradient_basis, weights
    )[0]
    assert loss >= 0.0
...
def test_length_smoothness_distinguishes_axis_and_diagonal_plane_fields() -> None:
    mesh = load_obj(ROOT / "data" / "plane.obj")
    areas, gradient_basis = face_gradient_basis(mesh)
    weights = areas / areas.sum()
    x = mesh.vertices[:, 0]
    ...
    x = (x - x.min()) / np.ptp(x)
    z = (z - z.min()) / np.ptp(z)
    axis_loss = length_smoothness_loss_and_gradient(
        x, mesh.faces, gradient_basis, weights
    )[0]
    diagonal_loss = length_smoothness_loss_and_gradient(
